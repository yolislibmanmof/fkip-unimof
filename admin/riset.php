...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if ($action === 'delete' && $id) {
        $stmt = $pdo->prepare("SELECT file_pdf FROM riset WHERE id = ?"); $stmt->execute([$id]);
        delete_upload($stmt->fetchColumn(), 'riset');
    ...
    header('Location: riset.php?' . http_build_query($_GET)); exit;
}

$q = trim($_GET['q'] ?? ''); $kat_filter = $_GET['kategori'] ?? '';
$where = 'WHERE 1=1'; $params = [];
if ($q !== '') { $where .= ' AND (judul LIKE ? OR penulis LIKE ?)'; $params[] = "%$q%"; $params[] = "%$q%"; }
if ($kat_filter !== '') { $where .= ' AND kategori = ?'; $params[] = $kat_filter; }

$stmt = $pdo->prepare("SELECT * FROM riset $where ORDER BY tahun DESC, created_at DESC");
$stmt->execute($params); $riset_list = $stmt->fetchAll();
...
$kat_stats = [];
foreach ($pdo->query("SELECT kategori, COUNT(*) as total FROM riset GROUP BY kategori")->fetchAll() as $r) $kat_stats[$r['kategori']] = (int)$r['total'];

$csrf = generate_csrf_token();
$active_menu = 'riset'; $page_heading = 'Kelola Riset';
...
<div class="stats-extreme" data-aos="fade-up">
    <div class="stat-card-extreme" style="--stat-color: #3b82f6;"><div class="stat-icon-extreme">🔬</div><div class="stat-number-extreme count-up" data-target="<?= $stat_total ?>">0</div><div class="stat-label-extreme">Total Riset</div></div>
    <div class="stat-card-extreme" style="--stat-color: #10b981;"><div class="stat-icon-extreme">✅</div><div class="stat-number-extreme count-up" data-target="<?= $stat_pub ?>">0</div><div class="stat-label-extreme">Dipublikasi</div></div>
    <div class="stat-card-extreme" style="--stat-color: #f59e0b;"><div class="stat-icon-extreme">📅</div><div class="stat-number-extreme count-up" data-target="<?= $stat_year ?>">0</div><div class="stat-label-extreme">Tahun Ini</div></div>
</div>

<?php if (!empty($kat_stats)): ?>
<div class="chart-section" data-aos="fade-up">
    <div class="chart-card"><h3>📊 Kategori Riset</h3><div id="katChart"></div></div>
    <div class="chart-card"><h3> Status Publikasi</h3><div id="statusChart"></div></div>
</div>
<?php endif; ?>

<div class="toolbar-extreme" data-aos="fade-up">
    <div class="search-box"><span class="search-icon"></span><input type="text" id="searchInput" placeholder="Cari judul atau penulis..." value="<?= sanitize($q) ?>"></div>
    <select class="filter-select" id="katFilter">
        <option value="">Semua Kategori</option>
        <option value="Pendidikan" <?= $kat_filter === 'Pendidikan' ? 'selected' : '' ?>>🎓 Pendidikan</option>
        <option value="Teknologi" <?= $kat_filter === 'Teknologi' ? 'selected' : '' ?>> Teknologi</option>
        <option value="Sosial" <?= $kat_filter === 'Sosial' ? 'selected' : '' ?>>🤝 Sosial</option>
    </select>
...
<div class="table-container" data-aos="fade-up">
    <div class="table-header">
        <div><h2>🔬 Daftar Riset & Publikasi</h2><p style="font-size: 0.85rem; color: var(--text-muted); margin-top: 0.25rem;">Kelola publikasi ilmiah dosen & mahasiswa</p></div>
    ...
    <?php if (empty($riset_list)): ?>
        <div class="empty-state-extreme"><div class="empty-icon-extreme">🔬</div><h3>Belum ada data riset</h3><a href="riset-form.php" class="btn-action primary" style="margin-top: 1rem;"> Tambah Riset Pertama</a></div>
    <?php else: ?>
        <div class="table-wrapper">
            <table class="extreme">
                <thead><tr><th>Judul Riset</th><th>Penulis</th><th>Tahun</th><th>Kategori</th><th>Status</th><th style="text-align: right;">Aksi</th></tr></thead>
                <tbody>
                <?php foreach ($riset_list as $r): 
                    $status_class = $r['status'] === 'Published' ? 'badge-published' : 'badge-draft';
                ?>
                <tr>
                    <td><strong style="color: var(--text-primary);"><?= sanitize($r['judul']) ?></strong><?php if (!empty($r['file_pdf'])): ?><br><a href="<?= asset('uploads/riset/' . basename($r['file_pdf'])) ?>" target="_blank" style="font-size: 0.8rem; color: var(--primary); text-decoration: none;">📄 Lihat PDF</a><?php endif; ?></td>
                    <td style="color: var(--text-secondary);"><?= sanitize($r['penulis']) ?></td>
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <form method="POST" style="display:inline" onsubmit="return confirm('Ubah status riset?')"><input type="hidden" name="csrf_token" value="<?= sanitize($csrf) ?>"><input type="hidden" name="id" value="<?= $r['id'] ?>"><input type="hidden" name="action" value="toggle"><button class="btn-icon toggle" title="Toggle">🔄</button></form>
                            <form method="POST" style="display:inline" onsubmit="return confirm('Hapus riset ini?')"><input type="hidden" name="csrf_token" value="<?= sanitize($csrf) ?>"><input type="hidden" name="id" value="<?= $r['id'] ?>"><input type="hidden" name="action" value="delete"><button class="btn-icon delete" title="Hapus">️</button></form>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
...
<?php if (!empty($kat_stats)): ?>
new ApexCharts(document.querySelector("#katChart"), { series: <?= json_encode(array_values($kat_stats)) ?>, labels: <?= json_encode(array_keys($kat_stats)) ?>, chart: { type: 'donut', height: 280 }, colors: ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6'], plotOptions: { pie: { donut: { size: '70%' } } }, legend: { position: 'bottom' } }).render();
new ApexCharts(document.querySelector("#statusChart"), { series: [<?= $stat_pub ?>, <?= $stat_total - $stat_pub ?>], labels: ['Published', 'Draft'], chart: { type: 'pie', height: 280 }, colors: ['#10b981', '#f59e0b'], legend: { position: 'bottom' } }).render();
<?php endif; ?>

const searchInput = document.getElementById('searchInput'); const katFilter = document.getElementById('katFilter');
let searchTimeout;
...
function applyFilters() { const q = searchInput.value; const kat = katFilter.value; const url = new URL(window.location); if (q) url.searchParams.set('q', q); else url.searchParams.delete('q'); if (kat) url.searchParams.set('kategori', kat); else url.searchParams.delete('kategori'); window.location = url; }
</script>
